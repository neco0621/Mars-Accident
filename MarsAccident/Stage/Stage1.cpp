...
Stage1::Stage1(StageManager& manager) : StageBase(manager_),
	m_enemyInterval(0),
	m_shakeFrame(0),
	m_wipeFrame(0),
...
Vec2 Stage1::GetNearEnemyPos(Vec2 pos)
{
	//敵がいない場合は適当な座標を返す
	Vec2 result{ 0,0 };
	//最初の敵チェック済みフラグ
...
bool Stage1::AddShot(ShotBeam* pBeam)
{
	for (int i = 0; i < m_pBeam.size(); i++)
	{
		//使用中なら次のチェックへ
...
void Stage1::FadeInUpdate(Input& input)
{
	frame_--;
	if (frame_ <= 0) {//遷移条件
		updateFunc_ = &Stage1::NormalUpdate;
		drawFunc_ = &Stage1::NormalDraw;
	}
}

void Stage1::NormalUpdate(Input& input)
{
	if (input.IsTriggered("OK"))
	{
		updateFunc_ = &Stage1::FadeOutUpdate;
		drawFunc_ = &Stage1::FadeDraw;
	}
}

void Stage1::FadeOutUpdate(Input& input)
{
	frame_++;
	if (frame_ >= 60) {
		manager_.ChangeStage(std::make_shared<Stage2>(manager_));
	}
}

void Stage1::FadeDraw()
{
	//フェード暗幕
